...
### Groping DNS
![[Pasted image 20260601181824.png]]
This output is the result of a DNS query using the `dig` command for the domain `google.com`. The command was executed on a system running `DiG` version 9.20.22-1 Debian. The output can be broken down into four key sections:

1. Header
    - `;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: 11913`: This line indicates the type of query (`QUERY`), the successful status (`NOERROR`), and a unique identifier (`11913`) for this specific query.
...
![[Pasted image 20260601182130.png]]
